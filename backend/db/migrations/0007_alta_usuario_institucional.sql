-- Alta directa de personal y usuarios institucionales. El rol estudiante queda
-- reservado al acto de inscribirse en un curso.

set local search_path = public, pg_catalog;

create function app.crear_usuario_institucional(
  p_nombres   text,
  p_apellidos text,
  p_correo    citext,
  p_telefono  text,
  p_codigo    text,
  p_rol       rol_institucional
)
...
begin
  ...
  if not app.es_admin() then
    raise exception 'solo administracion puede registrar usuarios'
      using errcode = '42501';
  end if;
  ...
  select u.id into v_usuario
    from usuarios u
   where u.correo = p_correo and u.eliminado_en is null
   for update;
  ...
  if exists (
    select 1 from membresias m
     where m.institucion_id = v_institucion
...
comment on function app.crear_usuario_institucional(text, text, citext, text, text, rol_institucional) is
  'Crea o afilia directamente un usuario con un rol institucional que no sea estudiante ni propietario.';

grant execute on function app.crear_usuario_institucional(text, text, citext, text, text, rol_institucional) to educa_app;
